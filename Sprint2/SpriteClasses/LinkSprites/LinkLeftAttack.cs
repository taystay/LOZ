...
namespace Sprint2.SpriteClasses.LinkSprites
{
    class LinkLeftAttack : ISprite
    {
        ...
        public LinkLeftAttack(Texture2D sprite)
        {
            linkSprite = sprite;
            frame = 0;
        }
        ...
        public void Draw(SpriteBatch spriteBatch, Point location)
        {
            ...
            if (frame == 0)
            {
                sourceRectangle = new Rectangle(30, 30, frameOneWidth, frameOneHeight);
                destinationRectangle = new Rectangle((int)location.X, (int)location.Y, frameOneWidth * scale, frameOneHeight * scale);
            }
            else
            {
                sourceRectangle = new Rectangle(24, 90, frameTwoWidth, frameTwoHeight);
                destinationRectangle = new Rectangle((int)location.X - (12 * scale), (int)location.Y, frameTwoWidth * scale, frameTwoHeight * scale);
            }
